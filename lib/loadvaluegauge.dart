...
class LoadValueGauge extends StatelessWidget {
  final int LoadValue;
  LoadValueGauge(this.LoadValue);

  @override
  Widget build(BuildContext context) {
    print("batteryvalue1 ${LoadValue}");
    ...
    return Stack(
      ...
      children: [
        CircularArc(key: UniqueKey(), LoadValue: LoadValue),
        Positioned(
...
class CircularArc extends StatefulWidget {
  final int LoadValue;
  const CircularArc({Key? key, required this.LoadValue}) : super(key: key);
...
class _CircularArcState extends State<CircularArc>
  ...
  void initState() {
    super.initState();
    print("batteryvalue inside ${widget.LoadValue}");
    calculation = widget.LoadValue / 2000;
    beginvalue = 0.0;
...
class ProgressArc extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Color arcColor = (DataHolder.LoadValue > 100 && DataHolder.LoadValue < 1900)
        ? Colors.green
        : Colors.red;
