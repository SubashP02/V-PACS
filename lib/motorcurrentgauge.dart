...
class MotorCurrentGauge extends StatelessWidget {
  final int MotorCurrent;
  MotorCurrentGauge(this.MotorCurrent);

  @override
  Widget build(BuildContext context) {
    print("batteryvalue1 ${MotorCurrent}");
    ...
    return Stack(
      ...
      children: [
        CircularArc(key: UniqueKey(), MotorCurrent: MotorCurrent),
        Positioned(
          ...
          child: CircleAvatar(
            ...
            child: Text(
              '${MotorCurrent.toStringAsFixed(0)}',
              style: TextStyle(
...
class CircularArc extends StatefulWidget {
  final int MotorCurrent;
  const CircularArc({Key? key, required this.MotorCurrent}) : super(key: key);
...
class _CircularArcState extends State<CircularArc>
  ...
  void initState() {
    super.initState();
    print("batteryvalue inside ${widget.MotorCurrent}");
    calculation = widget.MotorCurrent / 1500;
    beginvalue = 0.0;
...
class ProgressArc extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Color arcColor = (DataHolder.MotorCurrent > 100 && DataHolder.MotorCurrent < 1400)
        ? Colors.green
        : Colors.red;
