...
class MotorTempGauge extends StatelessWidget {
  final int MotorTemp;
  MotorTempGauge(this.MotorTemp);

  @override
  Widget build(BuildContext context) {
    print("batteryvalue1 ${MotorTemp}");
    ...
    return Stack(
      ...
      children: [
        CircularArc(key: UniqueKey(), MotorTemp: MotorTemp),
        Positioned(
...
class CircularArc extends StatefulWidget {
  final int MotorTemp;
  const CircularArc({Key? key, required this.MotorTemp}) : super(key: key);
...
class _CircularArcState extends State<CircularArc>
  ...
  void initState() {
    super.initState();
    print("batteryvalue inside ${widget.MotorTemp}");
    calculation = widget.MotorTemp / 80;
    beginvalue = 0.0;
...
class ProgressArc extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Color arcColor = (DataHolder.MotorTemp > 0 && DataHolder.MotorTemp < 60)
        ? Colors.green
        : Colors.red;
